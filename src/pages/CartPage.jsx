  ...
  if (cart.length === 0) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center px-6 text-center">
        ...
        <Link
          to="/shop"
          className="bg-deli-red text-white px-8 py-4 rounded-full font-sans text-[10px] uppercase tracking-widest font-bold"
        >
          Explore Apothecary
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          {cart.map((item) => (
            <div
              key={item.id}
              className="flex gap-6 items-center border-b border-deli-charcoal/5 pb-8"
            >
              {/* Product Image */}
              ...
              <div className="flex-1">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center border border-deli-charcoal/10 rounded-full px-2 py-1">
                    ...
                    <span className="px-4 font-sans text-xs min-w-[30px] text-center">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => updateQuantity(item.id, item.quantity + 1)}
